...
## Before You Start
- [x] 1. Click `Use this template` and save as your own repo
- [x] 2. Clone your repo, I prefer the open in GitHub Desktop method
- [x] 3. Open the cloned repo in VS Code

## Docker Setup
- [x] 1. Make sure you have Docker installed on your machine. You can download it from the official Docker website. Here is the link: https://docs.docker.com/get-docker/

## Setup your .env file
- [x] 1. Edit the `editme.env` by renaming it to just `.env`
- [x] 2. Your instructor will share the `SNOWFLAKE_ACCOUNT` value in Slack. It uses the `ORG-ACCOUNT` format (e.g. `VXZJSRC-AG60135`), not the retired `SFEDU02` account or the older `xyz12345.us-west-2` locator. If you look up your account in Snowsight, the snippet there may include `authenticator = "externalbrowser"` — ignore that line; this stack uses key-pair auth (next section).
...
- [x] 1. Open a new terminal in VS Code -> Terminal -> New Terminal
- [x] 2. Run this code in the terminal
```bash
pip install cryptography paramiko==3.5.1 python-dotenv sshtunnel==0.4.0
```

- [x] 3. Run the `airflow-core-fernet-key.py` script to generate a fernet key. This key is used to encrypt sensitive data in Airflow, such as passwords and connection strings.
```bash
python airflow-core-fernet-key.py
```

- [x] 4. Copy the generated fernet key and paste it into the `.env` file in the `FERNET_KEY` variable.

<br>

## (NA) WINDOWS Generate a Public Key for Snowflake
...
- [x] 1. Generate SSH Keys for Snowflake Connection. Run the following commands in a `terminal shell`.
...
- [x] 2. In your `.env` file, set `HOST_USERNAME` to your Mac short name (the one shown in `/Users/<this>`) and update `SNOWFLAKE_PRIVATE_KEY_PATH` to match. Both values must use the **same** username:
```
HOST_USERNAME=your-username
SNOWFLAKE_PRIVATE_KEY_PATH=/Users/your-username/.ssh/dbt_key.p8
```

- [x] 3. Your public key is now copied to your clipboard — paste it when prompted by your Snowflake admin (your teacher) to set up key pair authentication.

Resource: [Snowflake Documentation on Key Pair Auth](https://docs.snowflake.com/en/user-guide/key-pair-auth)

<br>

## (NA) LINUX Generate a Public Key for Snowflake
...
- [x] 1. Make sure the docker app is open on your machine
- [x] 2. Open a new terminal in VS Code -> Terminal -> New Terminal
- [x] 3. Run this code in the terminal
```bash
...
- [x] 4. Open [http://localhost:8080](http://localhost:8080)
...
- [x] 1. Run this in the terminal in VS Code
```bash
docker compose down
```
